...
OrcaAgentRunner = Callable[..., Awaitable[RunOutcome]]
...
def list_agent_definitions() -> list[AgentDefinition]:
  ...
  return [
      AgentDefinition(
          id="orca",
          runtime="native",
          entrypoint="src.agents.orca.runner:run",
          label="Orca",
...
def load_agent(agent_id: str) -> OrcaAgentRunner:
  normalized = validate_agent_id(agent_id)
  if normalized == "orca":
    from .runner import run

    return run
  raise ValueError(f"Unknown Orca agent_id: {normalized}")
...
def validate_agent_id(agent_id: str) -> str:
  ...
  if normalized != "orca":
    raise ValueError(f"Unknown Orca agent_id: {normalized}. Available: orca.")
  return normalized
