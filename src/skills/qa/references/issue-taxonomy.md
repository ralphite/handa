...
Reference for categorizing issues found during QA. Read this at the start of a QA session to calibrate what to look for.
...
- Failed navigations
- Broken images or assets visible in the UI
- Failed requests that surface through UI messages
- Console or network errors only when the browser agent explicitly reports that this data is available
...
6. **Error states** -- Check visible errors, failed navigations, and surfaced request failures. Check console/network data only if the browser agent exposes it.
